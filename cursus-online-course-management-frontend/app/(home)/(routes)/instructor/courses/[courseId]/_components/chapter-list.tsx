...
interface ChaptersListProps {
  items: Lesson[];
  onReorder: (updateData: { lessonId: string; position: number }[]) => void;
...
const ChaptersList = ({ items, onReorder, onEdit }: ChaptersListProps) => {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const bulkUpdateData = updatedChapters.map((chapter) => ({
      ...
      // Add 1 to the position to start counting from 1
      position: items.findIndex((item) => item.lessonId === chapter.lessonId) + 1,
    }));
                ...
                {(provided) => (
                  ...
                  >
                    ...
                    <div className="ml-auto pr-2 flex items-center gap-x-2">
                      ...
                      <Pencil
                        onClick={() => onEdit(chapter.lessonId)}
